...
### 2.10 Create a backend Service

The UI (and `oc port-forward`) needs a Kubernetes Service that routes to the
sandbox pod's TCP proxy on port 8000.

```shell
# Label the sandbox pod
oc label pod aiq-ocp -n openshell app=aiq-backend

# Create the Service
cat <<'EOF' | oc apply -n openshell -f -
apiVersion: v1
kind: Service
metadata:
  name: aiq-backend
...
```shell
oc get endpoints aiq-backend -n openshell
# Should show the pod IP on port 8000
```

### 2.11 (Optional) Deploy the UI

Push the UI image to the internal registry:

```shell
REGISTRY=$(oc get route default-route -n openshift-image-registry -o jsonpath='{.spec.host}')
docker tag aiq-ui:local $REGISTRY/openshell/aiq-ui:latest
docker push $REGISTRY/openshell/aiq-ui:latest
```

Create the UI Deployment, Service, and Route:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: ui
          image: image-registry.openshift-image-registry.svc:5000/openshell/aiq-ui:latest
          ports:
            - containerPort: 3000
          env:
            - name: BACKEND_URL
              value: "http://aiq-backend.openshell.svc:8000"
            - name: REQUIRE_AUTH
              value: "false"
            - name: NODE_ENV
              value: "production"
...
```shell
oc get route aiq-ui -n openshell -o jsonpath='{.spec.host}'
```
...
| `openshell` | Gateway StatefulSet (`openshell-0`), sandbox pods (`aiq-ocp`), `aiq-backend` Service, `aiq-ui` Deployment/Service/Route, service accounts (`openshell`, `openshell-sandbox`), JWT signing secret, ConfigMap, PVCs |
...
```shell
# Delete the UI
oc delete deployment,svc,route -l app=aiq-ui -n openshell

# Delete the backend Service
oc delete svc aiq-backend -n openshell
...
| **UI** | `docker run` on host network | Deployment + Service + Route with edge TLS (§2.11) |
